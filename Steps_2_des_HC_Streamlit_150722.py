...
class g:
    with st.sidebar:
        ...
        prob_prefer_f2f = st.number_input("Probability a patient will prefer "
                                          ...
                                          min_value = 0.00, max_value=1.00, 
                                          value=0.208)
        prob_priority = st.number_input("Probability a patient will be a priority" 
                                        " patient", min_value = 0.0, max_value=1.0, 
                                        value=0.14)
        prob_refer_ieso = st.number_input("Probability a patient will be referred"
                                          " to IESO", min_value = 0.0, 
                                          max_value=1.0, value=0.18)
        prob_refer_121 = st.number_input("Probability a patient will be referred "
                                         "to 121", min_value = 0.0, max_value=1.0, 
                                         value=0.63)
        prob_refer_group = st.number_input("Probability a patient will be referred"
                                           " to group", min_value = 0.0, 
                                           max_value=1.0, value=0.19)
        mean_wait_ieso = st.number_input("Mean number of weeks patients wait for "
                                         "IESO", min_value = 0.0, max_value = 52.0,
                                         value = 5.93)
        sess_no_group = st.number_input("(Mean) number of sessions for a full "
                                        "course of group therapy", min_value = 1, 
                                        max_value = 25, value = 12)
        ...
        mean_sess_no_ieso = st.number_input("Mean number of sessions patients "
                                            "undertake for IESO", min_value = 1.0, 
                                            max_value = 30.0, value = 9.41)
        num_group_spaces = st.number_input("Number of spaces available on each "
                                           "group", min_value = 2, max_value = 20, 
                                           value = 6)
        num_eve_groups = st.number_input("Number of evening groups running per week", 
                                         min_value = 0, max_value = 30, value = 4)
        num_day_groups = st.number_input("Number of daytime groups running per week", 
                                         min_value = 0, max_value = 30, value = 15)    
        num_app_121_f2f_eve = st.number_input("Number of evening face to face 121 "
        ...
        num_app_121_v_eve = st.number_input("Number of evening virtual 121 "
                                            ...
                                            value = 44)
        num_app_121_f2f_day = st.number_input("Number of daytime face to face 121 "
                                              "appointments available per week", 
                                              min_value = 0, max_value = 100, 
                                              value = 77)
        num_app_121_v_day = st.number_input("Number of daytime virtual appointments"
                                            " available per week", min_value = 0, 
                                            max_value = 400, value = 293)
    inter_arrival = 1/mean_new
    num_app_ieso = 9999999 #unlimited number of ieso appointments
    num_app_group_eve = num_group_spaces*num_eve_groups 
    #num evening group appointments available per week
    num_app_group_day = num_group_spaces*num_day_groups
    #num daytime group appointments available per week
...
class Patient:
    def __init__(self, p_id):
        ...
        #does patient prefer evening appointments? (group & 121 patients only)
        if self.treatment == "121" or "group":
            if x < g.prob_prefer_eve:
                self.eve_prefer = True
            else:
                self.eve_prefer = False
       
        #does patient prefer F2F appointments? (121 patients only)
        if self.treatment == "121":
            if x < g.prob_prefer_f2f:
                self.f2f_prefer = True
            else:
                self.f2f_prefer = False
...
if st.button("Run"):
    ...
    wait_by_priority_stats.rename(index={0.0:'Not Priority', 1.0:'Priority'}, 
                                  inplace=True)
